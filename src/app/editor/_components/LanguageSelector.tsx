...
function LanguageSelector({ hasAccess }: { hasAccess: boolean }) {
    ...
    const handleLanguageSelect = (langId: string) => {
        if (!hasAccess && langId !== "javascript" && langId !== "typescript") return;
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <motion.button
                ...
                className={`group relative flex items-center gap-3 px-4 py-2.5 bg-[#1e1e2e]/80 rounded-lg transition-all duration-200 border border-gray-800/50 hover:border-gray-700 ${!hasAccess && language !== "javascript" && language !== "typescript" ? "opacity-50 cursor-not-allowed" : ""}`}
            >
                {/* Decoration */}
                <div
