...
import DeleteSnippetDialog from "@/app/snippets/_components/DeleteSnippetDialog";
...
function SnippetsPage() {
    ...
    const [isDeleteSnippetDialogOpen, setIsDeleteSnippetDialogOpen] = useState(false);
    const [isDeleting, setIsDeleting] = useState(false);
    const [selectedLanguage, setSelectedLanguage] = useState<string | null>(null);
    const [view, setView] = useState<"grid" | "list">("grid");
    const deleteSnippet = useMutation(api.snippets.deleteSnippet);
    const [deleteId, setDeleteId] = useState<Id<"snippets">>();
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0f]">
                    ...
                    <AnimatePresence mode="popLayout">
                        {filteredSnippets.map((snippet) => (
                            <SnippetCard
                                key={snippet._id}
                                snippet={snippet}
                                setDeleteDialogOpen={setIsDeleteSnippetDialogOpen}
                                setDeleteId={()=>setDeleteId(snippet._id)}
            ...
            {
                isDeleteSnippetDialogOpen &&
                <DeleteSnippetDialog
                    onClose={()=>setIsDeleteSnippetDialogOpen(false)}
                    id={deleteId}
                    setIsDeleting={setIsDeleting}
                />
            }
